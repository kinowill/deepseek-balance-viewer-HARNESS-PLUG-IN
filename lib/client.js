 ...
 * details, refresh, and API-key management (via the host `balance` Remote).
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var tagId = "dsh-deepseek-balance/style";
    if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + tagId + '"]') === null) {
      var styleTag = document.createElement("style");
      styleTag.dataset.plugin = "dsh-deepseek-balance";
    ...
    // --------------------------------------------------------------- plugin --
    var inject = ["slots", "locale", "remote", "remote.balance"];
    ...
    function apply(ctx) {
      ...
      var port = {
        ...
        setApiKey: function (key) {
          return ctx.remote.balance.setApiKey(key).then(function () {
            store.refresh();
          });
        },
        removeApiKey: function () {
          return ctx.remote.balance.removeApiKey().then(function () {
            store.refresh();
          });
        },
        describeKey: function () {
          return ctx.remote.balance.describeKey();
        },
      };
      ...
      ctx.effect(
        function () {
          ctx.remote.$on("credentials/reference-updated", function () {
            store.refresh();
          });
        },
